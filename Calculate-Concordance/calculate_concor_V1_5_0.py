...
### python [3.8.19]
### matplotlib [3.7.5]
### Copyright: Baiqi Jiang & Or Yaacov @2024
...
logging.info("Running calculate_concor (V1.5.0)...")
...
def calculate_concordance(truth_vcf_path, call_vcf_path, sample_truth, sample_call, info_field, intervals, output_path, stat_counts, generate_figs, figs_path, get_info_from_call):
    ...
    for truth_var in truth_vcf:
        ...
        if true_variant_key in call_variants:
            ...
            variant_freq = matched_var.INFO.get(info_field) if get_info_from_call else truth_var.INFO.get(info_field)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--get_info_from_call", type = bool, default = False, help = "Default is False, when True, get INFO from call VCF instead of truth VCF.")
    ...
    calculate_concordance(args.truth_vcf, args.call_vcf, args.sample_truth, args.sample_call, args.info_field, args.intervals, args.output_path, args.stat_counts, args.generate_figs, args.figs_path, args.get_info_from_call)

    end_time = datetime.datetime.now()
    total_time = end_time - start_time
    logging.info(f"calculate_concor (V1.5.0) finished running successfully. Total runtime: {total_time}")
